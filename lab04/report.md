...
### Приклад, де без перевірки була б помилка часу виконання

Для `x = 2` програма в гілці `x > 2` перевіряє `ln(x - 2)`, але якщо викликати логарифм без перевірки для `x = 2`, то `x - 2 = 0`, а `ln(0)` є невизначеним (`-inf`), що дає неприпустимий результат і може призвести до `nan` у подальших обчисленнях.
